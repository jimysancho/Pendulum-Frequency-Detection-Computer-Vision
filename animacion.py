...
def frequency_probability(i):
    ...
    p_w = information[2]
    w_range = information[5]
    w = pendulum.linspace(w_range, len(p_w))

    plt.cla()
    plt.title(rf'$B_{{{i}}}$ = {np.round(w[np.argmax(p_w)], decimals=3)}')
    plt.scatter(w, p_w, s=20, color='red', alpha=0.8)
    plt.scatter(w[np.argmax(p_w)], np.max(p_w), edgecolor='black', color='blue', s=50)
    plt.xlabel('Amplitud 2')
    plt.ylabel(r'p(B)')
...
if __name__ == "__main__":
    ...
    # animation tools
    function = frequency_probability
    ...
    if animate:
        x_vals = []
        y_vals = []
        path = '/Users/Jaime/Desktop/TFG/Memoria/Vídeos/distribucion_amplitud_cos.gif'
