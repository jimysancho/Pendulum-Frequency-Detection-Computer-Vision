import cv2 as cv
import numpy as np
from Interactive_plot import CV_PLOT
...
while run:
    ...
    if stop:
        ...
        cv_plot = CV_PLOT(frame, point_size=size)
    ...
    if ok:
        ...
        cv.circle(frame, (X + x1, Y + y1), 10, (0, 255, 0), -1)
        ...
        cv.imshow('roi', gray_pendulum)
        
    if cv_plot is not None:
        cv_plot.trajectory(points, live=True, final=False)
        cv_plot.update(points, True)
        
    putText(frame,method)
    cv.imshow(NAME,frame)
    
show = True
if cv_plot is not None and show:
    cv_plot.trajectory(points, live=True)
    
save = False

if save and len(points) > 100:
    points = pd.DataFrame(points, columns=['x', 'y'])
    points.to_csv('Datos/2a-tanda-Foucault.csv', index=False)
